...
# Set keys
openai.api_key = secrets.get("OPENAI_API_KEY", os.environ.get("OPENAI_API_KEY", ""))
...
SERVICE_ACCOUNT_FILE = os.environ.get('GOOGLE_CREDENTIALS_PATH', 'austin-hybrid-battery-chatbot-354644e1d432.json')
...
def handle_booking_mode(message, state):
    ...
    if not state.get("vehicle"):
        ...
        if match:
            ...
            return "Got it! Just confirming everything now..."
        else:
    ...
    if all(state.values()):
        ...
        if message.strip().upper() == "BOOK NOW":
            ...
            # Reset to chat mode after booking
...
def chat(user_msg):
    ...
    st.session_state.setdefault("booking_state", {
        "vehicle": None,
        "datetime": None,
        "name": None,
        "phone": None,
        "duration": None,
        #"confirmation_requested": False #THIS WASNT IN ORIGINAL. REMOVE IF PROBLEMATIC
    })
    ...
    if st.session_state["mode"] == "booking":
        response = handle_booking_mode(user_msg, st.session_state["booking_state"])
        if not response:
            # If partial info gathered
            for key, val in st.session_state["booking_state"].items():
                if val is None:
                    prompts = {
                        ...
                        "datetime": get_datetime_prompt(st.session_state["booking_state"]["duration"]),
        ...
        response = handle_chat_mode(st.session_state["history"])

    st.session_state["history"].append({"role": "assistant", "content": response})
    #session.modified = True #THIS MAYBE NEED TO BE CHANGED
